...
internal class IrGraphGenerator(
  ...
  // Legacy pre-sharding extension delegation logic. This has been superseded by the post-sharding
  // delegation synthesis implemented in ShardedGraphGenerator.synthesizeExtensionDelegations().
  // We keep the old logic behind this flag temporarily for ease of rollback / comparison while
  // stabilizing sharded fast-init. Disable by default to avoid double-generation and inconsistent
  // BindingFieldContext state leading to test instability.
  ...
  fun generate() =
    with(graphClass) {
      ...
      // If this is a generated graph extension and it has pre-synthesized delegated provider
      // fields from IrGraphExtensionGenerator, register them up-front so expression generation can
      // resolve them without needing to re-synthesize.
      (generatedGraphExtensionData != null).let { isExtension ->
  if (ENABLE_LEGACY_SHARDED_EXTENSION_DELEGATION && isExtension) {
          extensionDelegatedProviderFields?.forEach { fld ->
            // Infer a type key from provider generic argument
            val simple = fld.type as? IrSimpleType
            val underlying = simple?.arguments?.firstOrNull()?.typeOrFail
            if (underlying != null) {
              val key = bindingGraph.bindingsSnapshot().keys.firstOrNull { it.type == underlying } ?: IrTypeKey(underlying)
              if (bindingFieldContext.providerField(key) == null) {
                bindingFieldContext.putProviderField(key, fld)
              }
            }
          }
        }
      }
      // If this is a graph extension (subcomponent) of a sharded parent graph, we need to
      // generate delegation provider fields for any parent-owned bindings the subcomponent
      // references. Otherwise accessor/injector code generation may emit field accesses like
      // `this.analyticsTrackerProvider` that do not exist locally, causing NoSuchFieldError
      // at runtime (observed for analyticsTrackerProvider in metro-fastinit mode).
      //
      // Strategy (Option A): before generating provider fields for this (non-sharded) graph
      // extension, scan reachable bindings; for any key that is *not* owned by this graph but
      // is provided by an ancestor graph that *was sharded*, synthesize a private delegation
      // field:
      //   private val analyticsTrackerProvider: Provider<AnalyticsTracker> =
      //     Provider.Companion.provider { parentInstance.shardN.analyticsTrackerProvider.invoke() }
      // or if the parent field is on the component directly (no shard):
      //     provider { parentInstance.analyticsTrackerProvider.invoke() }
      // This mirrors cross-shard delegation logic in ShardedGraphGenerator while keeping
      // subcomponent code simple and avoiding changes to BindingFieldContext semantics.
  ...
  val delegatedFieldLogs = mutableListOf<String>()
        if (options.shardingDebug) {
          writeDiagnostic("sharding-trace.txt") {
            "ENTER extension-delegation primary pass for ${graphClass.name} parent=${parentClass.name}"
          }
        }
        ...
        val shouldGenerateDelegations = shardClasses.isNotEmpty() && graphClass != parentClass && !graphClass.name.asString().startsWith("Shard")
    if (ENABLE_LEGACY_SHARDED_EXTENSION_DELEGATION && shouldGenerateDelegations) {
          if (options.shardingDebug) {
            writeDiagnostic("sharding-trace.txt") {
              buildString {
                appendLine("[delegation] shouldGenerateDelegations=true shards=${shardClasses.size} graph=${graphClass.name}")
              }
            }
          }
          // Identify the captured outer instance field (inner class synthetic) if present
          val outerInstanceField = declarations.filterIsInstance<IrField>()
            .firstOrNull { it.type == parentClass.defaultType }
          data class ParentProvider(
            val providerField: IrField,
            val shardInstanceField: IrField?,
          )

          // GraphDependency bindings referencing parent graph. Use snapshot view.
          val graphDeps = bindingGraph.bindingsSnapshot().values
            .filterIsInstance<IrBinding.GraphDependency>()
            .filter { it.ownerKey != node.typeKey }

          for (dep in graphDeps) {
            if (bindingFieldContext.providerField(dep.typeKey) != null) continue
            val targetType = dep.typeKey.type
            if (options.shardingDebug) {
              writeDiagnostic("sharding-trace.txt") { "[graphDep-scan] key=${dep.typeKey.render(short = true)} searching provider on parent/shards" }
            }
            // Find direct provider on parent
            val direct = parentClass.declarations.filterIsInstance<IrField>()
              .firstOrNull { f ->
                (f.type as? IrSimpleType)?.classOrNull == symbols.metroProvider &&
                  (f.type as IrSimpleType).arguments.firstOrNull()?.typeOrFail == targetType
              }?.let { ParentProvider(it, null) }
            val fromShard = direct ?: shardClasses.firstNotNullOfOrNull { shard ->
              val providerInShard = shard.declarations.filterIsInstance<IrField>()
                .firstOrNull { f ->
                  (f.type as? IrSimpleType)?.classOrNull == symbols.metroProvider &&
                    (f.type as IrSimpleType).arguments.firstOrNull()?.typeOrFail == targetType
                }
              if (providerInShard != null) {
                val shardInstanceField = parentClass.declarations.filterIsInstance<IrField>()
                  .firstOrNull { it.type == shard.defaultType }
                ParentProvider(providerInShard, shardInstanceField)
              } else null
            }
            val location = direct ?: fromShard ?: continue
            val outerField = outerInstanceField
            if (options.shardingDebug) {
              writeDiagnostic("sharding-trace.txt") { "[graphDep-hit] key=${dep.typeKey.render(short = true)} fromShard=${fromShard!=null} providerField=${location.providerField.name}" }
            }

            // Reuse the exact parent provider field name to align with existing accessor code
            // generation patterns that expect the simple provider name like
            // `analyticsTrackerProvider` on the subcomponent. Using GraphDependency.nameHint would
            // prefix the graph name and break those references.
            val baseName = location.providerField.name.asString()
            val localFieldName = fieldNameAllocator.newName(baseName)
            val providerFieldType = targetType.wrapInProvider(symbols.metroProvider)
            val localField = addField(
              fieldName = localFieldName.asName(),
              fieldType = providerFieldType,
              fieldVisibility = DescriptorVisibilities.PRIVATE,
            )
            localField.initFinal {
              val parentInstance = if (outerField != null) {
                // Typical inner class synthetic field case
                irGetField(irGet(thisReceiverParameter), outerField)
              } else {
                // Fallback: directly reference outer class `this` value parameter. Inner classes
                // have implicit access to their outer instance. If this fails in IR validation,
                // we will revisit with a more explicit capture strategy.
                irGet(parentClass.thisReceiver!!)
              }
              val ownerForProvider = if (location.shardInstanceField != null) {
                irGetField(parentInstance, location.shardInstanceField)
              } else parentInstance
              val parentProviderFieldAccess = irGetField(ownerForProvider, location.providerField)
              val invoked = irInvoke(
                dispatchReceiver = parentProviderFieldAccess,
            ...
            delegatedFieldLogs += "graphDep:${dep.typeKey.render(short = true)} -> ${localField.name.asString()}"
            bindingFieldContext.putProviderField(dep.typeKey, localField)
          }

          // Fallback pass: some parent shard-provided bindings may not have been modeled as
          // GraphDependency entries (e.g., due to graph construction nuances). To avoid missing
          // local delegation fields that subcomponent field generation code will reference, scan
          // all shard provider fields and synthesize any missing delegations by underlying type.
          val existingLocalProviderTypes = declarations.filterIsInstance<IrField>()
            .mapNotNull { (it.type as? IrSimpleType)?.arguments?.firstOrNull()?.typeOrFail }
            .toSet()
          val allShardProviderFields = shardClasses.flatMap { shard ->
            shard.declarations.filterIsInstance<IrField>().mapNotNull { f ->
              val simple = f.type as? IrSimpleType ?: return@mapNotNull null
              if (simple.classOrNull != symbols.metroProvider) return@mapNotNull null
              val underlying = simple.arguments.firstOrNull()?.typeOrFail ?: return@mapNotNull null
              Triple(shard, f, underlying)
            }
          }
          for ((shard, providerField, underlyingType) in allShardProviderFields) {
            // Skip if local field with this name already exists
            if (declarations.any { it is IrField && it.name == providerField.name }) continue
            if (options.shardingDebug) {
              writeDiagnostic("sharding-trace.txt") { "[fallback-shard-scan] underlying=${underlyingType.render(short = true)} shard=${shard.name} providerField=${providerField.name}" }
            }
            val providerFieldType = underlyingType.wrapInProvider(symbols.metroProvider)
            val shardInstanceField = parentClass.declarations.filterIsInstance<IrField>()
              .firstOrNull { it.type == shard.defaultType }
            val localField = addField(
              fieldName = providerField.name,
              fieldType = providerFieldType,
              fieldVisibility = DescriptorVisibilities.PRIVATE,
            )
            localField.initFinal {
              val parentInstance = if (outerInstanceField != null) {
                irGetField(irGet(thisReceiverParameter), outerInstanceField)
              } else {
                irGet(parentClass.thisReceiver!!)
              }
              val owningShardInstance = if (shardInstanceField != null) {
                irGetField(parentInstance, shardInstanceField)
              } else parentInstance
              val parentProviderFieldAccess = irGetField(owningShardInstance, providerField)
              val invoked = irInvoke(
                dispatchReceiver = parentProviderFieldAccess,
                callee = symbols.providerInvoke,
            ...
            // Try to find existing key, else create a synthetic one matching underlying type.
            val key = bindingGraph.bindingsSnapshot().keys.firstOrNull { it.type == underlyingType }
              ?: IrTypeKey(underlyingType)
            if (bindingFieldContext.providerField(key) == null) {
              bindingFieldContext.putProviderField(key, localField)
            }
            delegatedFieldLogs += "fallback:${underlyingType.render(short = true)}@shard=${shard.name.asString()} -> ${localField.name.asString()}"
          }

          // Additional fallback: mirror direct parent provider fields (not in shards) that the
          // extension may reference. This covers cases like analyticsTrackerProvider which can
          // live directly on the parent graph impl rather than a shard. We only add ones that
          // don't already exist locally and aren't from shard provider fields we just processed.
          run {
            val shardProviderNames = allShardProviderFields.map { it.second.name }.toSet()
            ...
            for (providerField in parentProviderFields) {
              ...
              localField.initFinal {
                ...
                val parentProviderFieldAccess = irGetField(parentInstance, providerField)
                val invoked = irInvoke(
                  dispatchReceiver = parentProviderFieldAccess,
                  callee = symbols.providerInvoke,
                  typeHint = underlyingType,
              ...
              delegatedFieldLogs += "fallback-parent:${underlyingType.render(short = true)} -> ${localField.name.asString()}"
            }
          }
          if (options.shardingDebug) {
            writeDiagnostic("sharding-trace.txt") { "EXIT extension-delegation primary pass for ${graphClass.name} added=${delegatedFieldLogs.size}" }
          }
          if (delegatedFieldLogs.isNotEmpty() && options.shardingDebug) {
            writeDiagnostic("extension-delegations-${graphClass.name}.txt") {
              buildString {
                appendLine("Delegated provider fields for extension ${graphClass.name} (parent=${parentClass.name})")
                delegatedFieldLogs.forEach { appendLine(it) }
              }
            }
          }
        }
      ...
      // Secondary delegation pass: if this is a graph extension (subcomponent) ensure we mirror
      // any parent provider fields even if the parent graph was not sharded. Some fast-init
      // generated accessor logic assumes these fields exist locally on the extension impl.
      run {
        val parentClass = parentAsClass
        val isExtension = graphClass != parentClass && !graphClass.name.asString().startsWith("Shard")
        if (isExtension) {
          val outerInstanceField = declarations.filterIsInstance<IrField>()
            .firstOrNull { it.type == parentClass.defaultType }
          val existingNames = declarations.filterIsInstance<IrField>().map { it.name }.toSet()
          val parentProviderFields = parentClass.declarations.filterIsInstance<IrField>()
            .filter { f ->
              val simple = f.type as? IrSimpleType ?: return@filter false
              simple.classOrNull == symbols.metroProvider
            }
          val delegatedFieldLogs = mutableListOf<String>()
          if (options.shardingDebug) {
            writeDiagnostic("sharding-trace.txt") { "ENTER extension-delegation nonshard pass for ${graphClass.name} parent=${parentClass.name} parentProviders=${parentProviderFields.size}" }
          }
          for (providerField in parentProviderFields) {
            ...
            val underlyingType = simple.arguments.firstOrNull()?.typeOrFail ?: continue
            if (options.shardingDebug) {
              writeDiagnostic("sharding-trace.txt") { "[nonshard-scan] underlying=${underlyingType.render(short = true)} providerField=${providerField.name}" }
            }
            val providerFieldType = underlyingType.wrapInProvider(symbols.metroProvider)
            val localField = addField(
              fieldName = providerField.name,
            ...
            delegatedFieldLogs += "fallback-parent-nonshard:${underlyingType.render(short = true)} -> ${localField.name.asString()}"
          }
          if (delegatedFieldLogs.isNotEmpty() && options.shardingDebug) {
            writeDiagnostic("extension-delegations-nonshard-${graphClass.name}.txt") {
              buildString {
                appendLine("Delegated provider fields (non-shard parent) for extension ${graphClass.name} (parent=${parentClass.name})")
                delegatedFieldLogs.forEach { appendLine(it) }
              }
            }
          }
          if (options.shardingDebug) {
            writeDiagnostic("sharding-trace.txt") { "EXIT extension-delegation nonshard pass for ${graphClass.name} added=${delegatedFieldLogs.size}" }
          }
        }
      }
