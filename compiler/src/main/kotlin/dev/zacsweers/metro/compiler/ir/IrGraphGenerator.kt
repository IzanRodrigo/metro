...
// Replaced by [MetroConstants.MAX_INIT_STATEMENTS_PER_METHOD]. Retained no longer; migrate all
// usages to the shared constant to keep sharded and non-sharded strategies aligned.
...
internal class IrGraphGenerator(
  ...
  fun generate() =
    with(graphClass) {
      ...
      // Legacy early extension delegated provider registration removed; handled post-shard.
      ...
      // Legacy extension delegation passes removed (now handled exclusively post-shard in ShardedGraphGenerator)
      ...
      if (
        options.chunkFieldInits &&
          fieldInitializers.size + initStatements.size > MetroConstants.MAX_INIT_STATEMENTS_PER_METHOD
      ) {
        ...
        val chunks =
          buildList<IrBuilderWithScope.(thisReceiver: IrValueParameter) -> IrStatement> {
            ...
            .chunked(MetroConstants.MAX_INIT_STATEMENTS_PER_METHOD)
